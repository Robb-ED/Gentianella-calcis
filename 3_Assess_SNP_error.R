...
#Most modifications relate to removal of allele error calculation, minor tidy-ups of formatting, etc
...
SNP_error <- function(mat, param){ 
  ...
  ## replicate pairs should be labeled as a or _a
  ...
  reps <- grep("a|_a",samples,value=TRUE) # get the replicates 
  samps <- match(sub("a|_a","",reps),samples) # match against its sample 
  samps<- samples[samps] # get names
  pairs<-cbind(reps,samps) # put them side by side 
  pairs<- pairs[rowSums(is.na(pairs)) < 1,] # remove rows that contain NA to get only successful replicate-sample pairs
...
SNP_errorstats <- SNP_error(SNPs, "Stacks")
