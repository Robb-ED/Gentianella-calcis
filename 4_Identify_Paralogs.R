...
#plot H and D
HDplotResults %>% ggplot()+geom_point(aes(x=H,y=D), alpha = 0.1)+ylim(-25,25)
#plot H and ratio
HDplotResults %>% ggplot()+geom_point(aes(x=H,y=ratio), alpha = 1/10)

#Assign paralog or singleton status
HDplotResults$status <- ifelse(HDplotResults$H <= 0.5 & 
                                 HDplotResults$D <= 4.0 &
                                 HDplotResults$D >= -4.0, 'SINGLETON','PARALOG')

#plot H and D with singletons and paralogs highlighted
HDplotResults %>% ggplot()+geom_point(aes(x=H,y=D, col = status), alpha = 0.2)+ylim(-25,25)+
  scale_color_manual(values=c("#DA8051","deepskyblue"))

#create list of singletons
HDplotResults$status <- ifelse(HDplotResults$H <= 0.5 & 
                                 HDplotResults$D <= 4.0 &
                                 HDplotResults$D >= -4.0, 1, 0)
singletons <- HDplotResults[which(HDplotResults$status==1),]
paralogs <- HDplotResults[which(HDplotResults$status==0),]
paralogs <- unique(paralogs$CHROM)
paralogs <- as.data.frame(paralogs)

#List the paralogs
paralogs

#Write it to a file so they can be blacklisted in Stacks
write.table(paralogs, "loci_to_blacklist.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
